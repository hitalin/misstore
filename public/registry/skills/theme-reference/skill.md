...
name: テーマ文法リファレンス
version: 0.1.0
description: Misskey / NoteDeck カラーテーマの構造・props 記法・全プロパティを実装準拠で正確に答えるリファレンス。テーマの書き方の質問・既存テーマの読解に使う
author: "@hitalin"
authorUrl: "https://github.com/hitalin"
category: utility
mode: trigger
triggers: [テーマの書き方, テーマ文法, テーマの仕組み, テーマの構造, props]
tags: [theme, reference, syntax, misskey]
scope: global
license: MIT
---
...
仕様があるが、NoteDeck / misstore のコンパイラは未対応 (空文字になる)。
互換性のため使わず、`@参照` で代用する。
...
・既存テーマと同じ `id` — 上書きインストールになる。新作は新規 UUID
・`$[fg.color=...]` 等の MFM 記法との混同 — それは投稿の装飾。テーマとは別物
  (MFM の質問は「MFM文法リファレンス」(mfm-reference) の領分)

配色の選び方・バランスの取り方・モチーフの落とし込みは別スキル
「テーマ職人」(theme-maker) が持つ。「〜っぽいテーマ作って」はそちらの領分。
